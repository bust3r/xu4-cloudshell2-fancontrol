...
# curl sysstat smartctl
# systemctl restart cloudshell-lcd
#
# Improvements by Eric Draken:
# - Show internal and external IP addresses
# - Skip refreshing the LCD if the backlight is off
# - Show network speeds in KB/s
...
get_external_ip() {
        export EXTERNAL_IP=$(/usr/bin/curl -s https://ipinfo.io/ip)
}
...
while true; do
		...
		# If the backlight is on, then refresh
		if [[ "$(head -1 /sys/class/backlight/*/bl_power)" != "0" ]]; then
			# sleep
			sleep $REFRESH
			continue
		fi
        ...
        # Line IP Addresses
        echo -e "IP: $CBLUE$INTERNAL_IP$COFF Ext: $CBLUE$EXTERNAL_IP$COFF" > $oc
        ...
        do
                ...
                echo -e "R/W $CBLUE${DISK_R[$i]}$COFF / $CBLUE${DISK_W[$i]}$COFF KB/s" > $oc
